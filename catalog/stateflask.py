...
engine = create_engine(
    'sqlite:///stateslist.db', connect_args={'check_same_thread':False}, echo=True)
...
def editCountry(country_id):
    ...
    if creater_id.id != login_session['user_id']:
        flash("you cannot edit this country this belongs to %s" % (creater_id.name))
        return redirect(url_for('showCountry'))
    if request.method=='POST':
...
def newStateList(state_id):
    ...
    user_id=getUserInfo(login_session['user_id'])
    if creater_id.id != login_session['user_id']:
        flash("you cannot add this state This is belongs to %s" % (creater_id.name))
